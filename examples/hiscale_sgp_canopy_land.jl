...
function hiscale_land_model(grid, static, aero, atmosphere, radiation)
    ...
    ## The stress closure gets texture-class loam retention parameters, not the
    ## pedotransfer medians: the two-point matching fits α and n through the very
    ## suction heads the stress function spans (see `PlantAvailableWaterStress`).
    canopy = CanopyConductanceHumidity(FT;
        leaf_area_index = aero.tile_leaf_area_index,
        conductance = JarvisConductance(FT),
        moisture_stress = PlantAvailableWaterStress(FT; texture = :loam),
