...
@kernel function _deep_store_head!(Πᵈ, Mᵈ, h)
    i, j = @index(Global, NTuple)
    @inbounds begin
        θᵈ = deep_liquid_fraction(i, j, h, Mᵈ[i, j, 1])
        𝒮ᵈ = liquid_saturation(h.soil, θᵈ, i, j)
        Πᵈ[i, j, 1] = pressure_head(h.soil.retention_curve, 𝒮ᵈ)
    end
end

@kernel function _deep_store_step!(Mᵈ, Jᵈ_diag, Jˡᵇ, h, Δt, time)
    i, j = @index(Global, NTuple)
    @inbounds begin
        Mij = Mᵈ[i, j, 1]
        θᵈ  = deep_liquid_fraction(i, j, h, Mij)
        𝒮ᵈ  = liquid_saturation(h.soil, θᵈ, i, j)
        Πᵈ  = pressure_head(h.soil.retention_curve, 𝒮ᵈ)
        Kᵈ  = hydraulic_conductivity(h.hydraulic_conductivity, 𝒮ᵈ)
        Jᵈ  = deep_liquid_flux(h.drainage, Mij, θᵈ, 𝒮ᵈ, Πᵈ, Kᵈ, zero(Mij), time)
...
function time_step!(h::DeepWaterStore, land, Δt, time)
    ...
    launch!(arch, land.grid, :xy, _deep_store_head!,
            h.soil.deep_pressure_head, land.prognostic.deep_water_storage, h)
