...
NumericalEarth.DataWrangling.default_download_directory(::GLORYSDataset) = download_GLORYS_cache
...
NumericalEarth.DataWrangling.all_dates(::GLORYSStatic, var) = [nothing]
NumericalEarth.DataWrangling.all_dates(::GLORYSDaily, var) = range(DateTime("1993-01-01"), stop=DateTime("2021-06-30"), step=Day(1))
NumericalEarth.DataWrangling.all_dates(::GLORYSMonthly, var) = range(DateTime("1993-01-01"), stop=DateTime("2021-06-01"), step=Month(1))
...
NumericalEarth.DataWrangling.reversed_vertical_axis(::GLORYSDataset) = true

NumericalEarth.DataWrangling.available_variables(::GLORYSDataset) = GLORYS_dataset_variable_names
...
NumericalEarth.DataWrangling.dataset_variable_name(metadata::GLORYSMetadata) = GLORYS_dataset_variable_names[metadata.name]
...
function NumericalEarth.DataWrangling.metadata_filename(dataset::GLORYSDataset, name, date, region)
...
NumericalEarth.DataWrangling.inpainted_metadata_path(metadata::GLORYSMetadatum) = joinpath(metadata.dir, inpainted_metadata_filename(metadata))

function NumericalEarth.DataWrangling.dataset_location(::GLORYSDataset, name)
    name == :free_surface && return (Center, Center, Nothing)
    return (Center, Center, Center)
end

NumericalEarth.DataWrangling.is_three_dimensional(metadata::GLORYSMetadata) = metadata.name != :free_surface

NumericalEarth.DataWrangling.longitude_interfaces(::GLORYSMetadata) = (-180, 180)
NumericalEarth.DataWrangling.latitude_interfaces(::GLORYSMetadata) = (-80, 90)

function NumericalEarth.DataWrangling.z_interfaces(metadata::GLORYSMetadata)
