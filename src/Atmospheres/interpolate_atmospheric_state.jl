...
@inline @generated function update_tracer_states!(i, j, surface_atmos_state, atmos_tracers::NamedTuple{names}, atmos_args, t_itp) where names
    calls = [:(update_tracer_state!(i, j, surface_atmos_state.$name, atmos_tracers.$name, atmos_args, t_itp)) for name in names]
    return quote
        $(calls...)
        return nothing
    end
