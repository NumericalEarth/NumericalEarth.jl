...
@inline NumericalEarth.EarthSystemModels.thermodynamics_parameters(atmos::Nothing) = nothing
@inline NumericalEarth.EarthSystemModels.thermodynamics_parameters(atmos::PrescribedAtmosphere) = atmos.thermodynamics_parameters
@inline NumericalEarth.EarthSystemModels.surface_layer_height(atmos::PrescribedAtmosphere) = atmos.surface_layer_height
@inline NumericalEarth.EarthSystemModels.boundary_layer_height(atmos::PrescribedAtmosphere) = atmos.boundary_layer_height

# No need to compute anything here...
NumericalEarth.EarthSystemModels.update_net_fluxes!(coupled_model, ::PrescribedAtmosphere) = nothing
...
Oceananigans.prognostic_state(atmos::PrescribedAtmosphere) =
    (; clock = Oceananigans.prognostic_state(atmos.clock))

function Oceananigans.restore_prognostic_state!(atmos::PrescribedAtmosphere, state)
    Oceananigans.restore_prognostic_state!(atmos.clock, state.clock)
    update_state!(atmos)
    return atmos
end
