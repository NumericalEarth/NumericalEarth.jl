# Figure 7: Wind stress (zonal, meridional) and (optional) NCEP biases.
function fig07(caches, labels, cases)
    ...
    for (i, lab) in enumerate(labels)
        c = caches[lab]
        panel!(fig, [1, 2i-1], get_field(c, :zonal_wind_stress);
               title = "$lab: Zonal wind stress", colormap = :balance,
               colorrange = (-0.3, 0.3), label = "N/m²")
        panel!(fig, [2, 2i-1], get_field(c, :meridional_wind_stress);
               title = "$lab: Meridional wind stress", colormap = :balance,
               colorrange = (-0.3, 0.3), label = "N/m²")
        if has_ncep
            zonal_bias      = get_field(c, :zonal_wind_stress_bias_ncep)
            meridional_bias = get_field(c, :meridional_wind_stress_bias_ncep)
            !isnothing(zonal_bias) && panel!(fig, [3, 2i-1], zonal_bias;
                title = "$lab: τx - NCEP", colormap = :balance,
                colorrange = (-0.15, 0.15), label = "N/m²")
            !isnothing(meridional_bias) && panel!(fig, [4, 2i-1], meridional_bias;
