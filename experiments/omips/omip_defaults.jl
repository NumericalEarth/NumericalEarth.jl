...
function omip_simulation(grid; forcing_dir, restoring_dir, filename, restart_iteration)
    ...
    set!(ocean.model, T=Metadatum(:temperature; dir=restoring_dir, dataset=EN4Monthly(), date),
                      S=Metadatum(:salinity;    dir=restoring_dir, dataset=EN4Monthly(), date))
    ...
    function progress(sim)
        ...
        Tmax = maximum(sim.model.ocean.model.tracers.T)
        Tmin = minimum(sim.model.ocean.model.tracers.T)
        Smax = maximum(sim.model.ocean.model.tracers.S)
        Smin = minimum(sim.model.ocean.model.tracers.S)
        ...
        msg4 = @sprintf("extrema(T, S): (%.2f, %.2f) ᵒC, (%.2f, %.2f) psu ", Tmin, Tmax, Smin, Smax)
