...
Use `Pkg.add(url="https://github.com/CliMA/NumericalEarth.jl.git", rev="main")` to install the latest version of `NumericalEarth`.
For more information, see the [documentation for `Pkg.jl`](https://pkgdocs.julialang.org).

## Why? What's the difference between NumericalEarth and [Oceananigans](https://github.com/CliMA/Oceananigans.jl)?

Oceananigans is a general-purpose library for ocean-flavored fluid dynamics.
NumericalEarth implements a framework for driving realistic Oceananigans simulations with prescribed atmospheres, and coupling them to prognostic sea ice simulations.

### A core abstraction: `NumericalEarth.OceanSeaIceModel`

Our system for realistic modeling is anchored by `NumericalEarth.OceanSeaIceModel`, which encapsulates the ocean simulation, sea ice simulation, prescribed atmospheric state, and specifies how the three communicate.
...
A second core abstraction in NumericalEarth is `ocean_simulation`. `ocean_simulation` configures an Oceananigans model for realistic simulations including temperature and salinity, the TEOS-10 equation of state, boundary conditions to store computed air-sea fluxes, the automatically-calibrated turbulence closure `CATKEVerticalDiffusivity`, and the [`WENOVectorInvariant` advection scheme](https://doi.org/10.1029/2023MS004130) for mesoscale-turbulence-resolving simulations.

NumericalEarth also provides convenience features for wrangling datasets of bathymetry, ocean temperature, salinity, ocean velocity fields, and prescribed atmospheric states.

NumericalEarth is built on top of Oceananigans and [ClimaSeaIce](https://github.com/CliMA/ClimaSeaIce.jl), so it's important that NumericalEarth users become proficient with [Oceananigans](https://github.com/CliMA/Oceananigans.jl).
Note that though NumericalEarth is currently focused on hydrostatic modeling with `Oceananigans.HydrostaticFreeSurfaceModel`, realistic nonhydrostatic modeling is also within the scope of this package.
...
> Wagner, G. L. et al. (2026). NumericalEarth/NumericalEarth.jl: v0.8.10 (v0.8.10). Zenodo. https://doi.org/10.5281/zenodo.7677442
