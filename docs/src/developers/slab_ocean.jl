...
#
# This tutorial demonstrates how to implement a new ocean component for `CoupledModel`: a **slab ocean model**.
...
#
# To integrate a slab ocean into `CoupledModel`, we need to extend several methods from NumericalEarth's `CoupledModels.jl` and
...
using NumericalEarth.CoupledModels: ocean_surface_salinity, ocean_surface_velocities
...
# ##  Extend the CoupledModels.jl module
#
# In the CoupledModels.jl module, we define the thermodynamic properties of the ocean component as well as all the helper functions
# needed to retrieve the ocean state and surface state.

using NumericalEarth.CoupledModels

CoupledModels.reference_density(::SlabOcean) = 1025.0
CoupledModels.heat_capacity(::SlabOcean) = 3990.0
CoupledModels.ocean_surface_salinity(slab_ocean::SlabOcean) = Oceananigans.Fields.ConstantField(35.0)
CoupledModels.ocean_salinity(slab_ocean::SlabOcean) = Oceananigans.Fields.ConstantField(35.0)
CoupledModels.ocean_temperature(slab_ocean::SlabOcean) = slab_ocean.temperature
...
CoupledModels.update_net_fluxes!(coupled_model, slab_ocean::SlabOcean) =
...
coupled_model = NumericalEarth.CoupledModel(slab_ocean, sea_ice; atmosphere, interfaces)
