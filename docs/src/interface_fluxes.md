...
To motivate this tutorial, we first note that `NumericalEarth`'s [`OceanSeaIceModel`](@ref) has essentially two goals:
...
To compute fluxes with NumericalEarth, we build an `OceanSeaIceModel` with an atmosphere and ocean state
...
```@example interface_fluxes
using NumericalEarth.OceanSeaIceModels.InterfaceComputations: surface_specific_humidity
...
```@example interface_fluxes
using NumericalEarth.OceanSeaIceModels: MomentumBasedFrictionVelocity
