...
| ``\alpha^{\mathrm{eff}}`` | `αᵉᶠᶠ` | column albedo | Broadband shortwave albedo of the canopy + ground column (–) |
| ``\epsilon`` | `ϵ` | emissivity | Surface emissivity (–) |
| ``T^{\mathrm{eff}}`` | `Tᵉᶠᶠ` | radiating temperature | Two-source blend whose ``\sigma (T^{\mathrm{eff}})^4`` is the canopy + ground upwelling longwave (K) |
