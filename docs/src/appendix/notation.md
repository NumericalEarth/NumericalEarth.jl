...
One symbol here is shared with another land parameter and means something
...
models; it keeps the symbol of its source parametrization. The zero-plane
displacement ``d`` is the same quantity as in
[Canopy aerodynamic roughness](@ref) above — only the closure that computes it
differs.
