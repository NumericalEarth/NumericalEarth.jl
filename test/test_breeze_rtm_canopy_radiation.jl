...
@testset "Breeze RadiativeTransferModel driving the land interface" begin
    for arch in test_architectures
        ...
        @testset "CanopyAirSpace under an RTM on $A" begin
            ...
            𝒬ᵍ = Array(interior(interface.temperature.ground_heat_flux))
            @test Jᴱs ≈ -𝒬ᵍ

            Hˡᵉᵃᶠ = Array(interior(interface.temperature.canopy_sensible_heat))
            Hᵍ = Array(interior(interface.temperature.soil_sensible_heat))
            LEˡᵉᵃᶠ = Array(interior(interface.temperature.canopy_latent_heat))
            LEᵍ = Array(interior(interface.temperature.soil_latent_heat))
            @test Hˡᵉᵃᶠ .+ Hᵍ ≈ Array(interior(interface.fluxes.sensible_heat))
            @test LEˡᵉᵃᶠ .+ LEᵍ ≈ Array(interior(interface.fluxes.latent_heat))
            ...
            effective_temperature = Array(interior(interface.temperature.effective))
            time_step!(model, 1.0)
            @test Array(interior(rtm.upwelling_longwave_flux))[:, :, 1] ≈ σ .* effective_temperature .^ 4 rtol = 1e-3
