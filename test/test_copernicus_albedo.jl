...
@testset "Copernicus land albedo sample windows" begin
    ...
    # A ten-day period is stamped on its last day, so its value belongs four days earlier — the
    # first two periods of a month are ten days, the third is whatever the month has left.
