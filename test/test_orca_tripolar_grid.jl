...
orca_datasets = (ORCA1, ORCA2)
orca_sizes = ((362, 332), (180, 148))
bathymetry_names = ("Bathymetry", "bathy_metry")
...
for (n, DATASET) in enumerate(orca_datasets)
    ...
    @testset "$name Metadatum construction" begin
        ...
        @test bathy_meta.dataset isa DATASET
    ...
    @testset "ORCATripolarGrid with $name dataset on $(arch)" for arch in test_architectures
        ...
        @test underlying.Nx == Nx0
        @test underlying.Ny == Ny0
    ...
    @testset "ORCATripolarGrid ($name) without bathymetry on $(arch)" for arch in test_architectures
        ...
        @test !(grid isa ImmersedBoundaryGrid)
        @test grid.Nx == Nx0
        @test grid.Ny == Ny0 - default_south_rows_to_remove(dataset)
        @test grid.Nz == 5
    end
    ...
    @testset "ORCATripolarGrid ($name) with south_rows_to_remove on $(arch)" for arch in test_architectures
        ...
        underlying = grid.underlying_grid
        @test underlying.Nx == Nx0
        @test underlying.Ny == Ny0 - Nremove
        @test underlying.Nz == 5
    end
