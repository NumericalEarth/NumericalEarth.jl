...
using NumericalEarth.Lands: SlabLand, SlabEnergy, VariablySaturatedHydrology,
    InterceptingHydrology, SurfaceWaterStore, DeepWaterStore,
...
head(land, hydrology, 𝒮) = pressure_head(1, 1, land.grid, hydrology.retention_curve, 𝒮)
...
@testset "Slab and store reach hydrostatic equilibrium" begin
    for arch in test_architectures
        ...
        Π   = head(land, soil, scalar(land.saturation))
        Πᵈ  = head(land, soil, 𝒮ᵈ)
...
@testset "Composition with the pond and the canopy store" begin
    for arch in test_architectures
        FT = Float64
        grid = store_test_grid(arch, FT)
        hydrology = InterceptingHydrology(FT;
            soil = SurfaceWaterStore(FT; soil = DeepWaterStore(FT; soil = soil_hydrology(grid, FT), thickness = hᵈ)),
            leaf_area_index = 3.0)
