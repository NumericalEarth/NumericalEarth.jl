...
for arch in test_architectures
    @testset "Cross-dataset region support (snapshot path) [$(typeof(arch))]" begin
        ...
        @testset "ECCO4 BoundingBox loads the right window" begin
            ...
            md = Metadatum(:temperature; dataset=ECCO4Monthly(), date=DateTime(1993, 1, 1), region=bbox)
            ...
            λg = Array(λnodes(f.grid, Center()))
            φg = Array(φnodes(f.grid, Center()))
            ...
            T_field = Array(interior(f))[i_grid, j_grid, end]
            @test T_field ≈ T_ref  rtol=1e-3
        end

        @testset "ECCO4 Column extracts a single point" begin
            col = Column(150.0, 0.0)
            md = Metadatum(:temperature; dataset=ECCO4Monthly(), date=DateTime(1993, 1, 1), region=col)
            f = Field(md, arch; inpainting=nothing, cache_inpainted_data=false)
