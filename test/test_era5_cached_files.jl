...
using NumericalEarth.DataWrangling.ERA5: ERA5HourlySingleLevel, ERA5HourlyPressureLevels, ERA5_all_pressure_levels, hPa
...
level_value(λ, φ, p) = mod(λ, 360) + 1000φ + 1e6 * p / hPa

function write_era5_file(path, λ, φ; pressure_levels = nothing)
    NCDataset(path, "c") do ds
        ...
        if isnothing(pressure_levels)
            ...
            defDim(ds, "pressure_level", length(pressure_levels))
            defVar(ds, "pressure_level", pressure_levels ./ hPa, ("pressure_level",))
            defVar(ds, "t", [level_value(λ, φ, p) for λ in λ, φ in φ, p in pressure_levels, _ in 1:1],
...
function fake_retrieve(product, request, path)
    ...
    pressure_levels = haskey(request, "pressure_level") ? parse.(Float64, request["pressure_level"]) .* hPa : nothing
    return write_era5_file(path, λ, φ; pressure_levels)
end
...
@testset "Pressure levels are selected from a file holding more of them" begin
    retrieve_data = NumericalEarth.DataWrangling.retrieve_data
    read_file_coords = NumericalEarth.DataWrangling.read_file_coords
    levels = [850, 500, 200]hPa
    level_temperature(levels, region, dir) = Metadatum(:temperature; dataset = ERA5HourlyPressureLevels(pressure_levels = levels),
                                                       date, region, dir)

    mktempdir() do dir
        large = BoundingBox(longitude = (0, 20), latitude = (30, 50))
        download(level_temperature(ERA5_all_pressure_levels, large, dir); retrieve = fake_retrieve)

        for region in (large, BoundingBox(longitude = (5, 10), latitude = (35, 40)))
            metadatum = level_temperature(levels, region, dir)
            retrievals[] = 0
            download(metadatum; retrieve = fake_retrieve)
            @test retrievals[] == 0
            λ, φ = read_file_coords(metadatum)
            @test retrieve_data(metadatum) == [level_value(λ, φ, p) for λ in λ, φ in φ, p in levels]
        end
    end

    mktempdir() do dir
        metadatum = level_temperature([850, 825]hPa, nothing, dir)
        write_era5_file(joinpath(dir, metadatum.filename), 0:0.25:1, 40:-0.25:39; pressure_levels = [1000, 850, 500]hPa)
        @test_throws "holds no 825.0 hPa level" retrieve_data(metadatum)
    end
end
